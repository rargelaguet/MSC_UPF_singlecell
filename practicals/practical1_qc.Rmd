...
```{r}
pbmc.data <- Read10X(data.dir = "/Users/ricard/data/not_used/pbmc_scrna/filtered_gene_bc_matrices")
class(pbmc.data)
```
...
```{r}
seurat <- CreateSeuratObject(counts = pbmc.data, project = "pbmc3k")
seurat
```
...
```{r}
nReads.per.cell <- apply(rna.mtx,2,sum)
...
mt_percentage.per.cell <- colMeans(rna.mtx[mt.genes,])
...
**(Q) Are the QC metrics correlated? why?**
...
```{r}
plot(x=mt_percentage.per.cell, y=nGenes.per.cell)
plot(x=nReads.per.cell, y=nGenes.per.cell)
```
...
```{r}
cells.to.keep <- mt_percentage.per.cell<10 & nGenes.per.cell>400 & nReads.per.cell>100
...
```{r}
rna_scaled.mtx <- GetAssayData(seurat, "scale.data")
hist(apply(rna_scaled.mtx,1,mean))
hist(apply(rna_scaled.mtx,1,sd))
```
...
We can visually inspect those genes that correlate the most with the top PCs:
```{r}
...
```{r}
# saveRDS(seurat, file = "/Users/parra/Desktop/StegleGroup/CourseHeidelberg/pbmc_tutorial_1stpart_output.rds")
```
